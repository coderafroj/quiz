...
function JoinForm() {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/set-state-in-effect -- one-time prefill from a QR-scanned URL, not a render loop
    if (fromQr) setCode(fromQr.replace(/\D/g, "").slice(0, 6));
  }, [searchParams]);
