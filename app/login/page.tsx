  ...
  return (
    <form onSubmit={handleSubmit} className="card-frame p-8 w-full max-w-sm">
      ...
      <input
        ...
        className="w-full bg-transparent border border-border px-3 py-2.5 mb-2 text-sm focus:border-fg outline-none"
      />
      <div className="text-right mb-6">
        <Link href="/forgot-password" className="text-fg-dim hover:text-fg font-mono text-[10px] uppercase tracking-widest hover:underline">
          Forgot password?
        </Link>
      </div>
      ...
      <p className="text-center mt-6 font-mono text-xs text-muted">
        No account?{" "}
        <Link href={`/signup${nextUrl !== "/dashboard" ? `?next=${encodeURIComponent(nextUrl)}` : ""}`} className="text-fg hover:underline">
          Create one
        </Link>
      </p>
    </form>
